...
{-
The problem is NP-Hard, so a 'correct' implmentation will always be slow.
Implmentations using techniques like addition star chains will fail to find an answer on some inputs.
The first answer is a naive implementation with no optimizations.
Below is a second implementation of the same algorithm with minor optimizations and parallization.
-}
